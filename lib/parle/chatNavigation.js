...
const APP_PREFETCH_ROUTES = ['/', '/dashboard', '/journal', '/quotes', '/login', '/register']

export function prefetchChatExitRoutes(router) {
  if (!router?.prefetch) return
  APP_PREFETCH_ROUTES.forEach((path) => router.prefetch(path))
  const returnPath = getChatReturnPath('/')
  if (returnPath && !APP_PREFETCH_ROUTES.includes(returnPath)) {
