...
function buildTrainingPayload(text) {
  ...
  let trainingContent = scrubbed
  try {
    trainingContent = encrypt(scrubbed)
  } catch {
    /* dev fallback — store scrubbed plain text only */
  }
  return { scrubbed, trainingContent }
}
...
async function logGuestTrainingMessage({
  ...
  const { scrubbed, trainingContent } = buildTrainingPayload(text)
  ...
  try {
    ...
    await db.query(
      ...
      [token, safeRole, scrubbed, safeModeId, safeReplyKind, trainingContent],
    )
    await db.query(
