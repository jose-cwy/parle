...
let profileSchemaReady = null

async function ensurePreferenceProfileSchema() {
  if (!profileSchemaReady) {
    profileSchemaReady = db
      .query(`
...
async function getOrCreateProfile(userId) {
  await ensurePreferenceProfileSchema()
...
async function getUserChatSettings(userId) {
  try {
    const privacy = await getUserSettings(userId)
    const user = await db.query('SELECT last_session_summary FROM users WHERE id = $1', [userId])
    const personalisationEnabled = Boolean(privacy.personalisation_enabled)
    const profile = personalisationEnabled ? await getOrCreateProfile(userId) : null
    return {
      memory_enabled: privacy.memory_enabled,
