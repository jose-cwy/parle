...
const { sanitizeJournalContent } = require('./security/sanitize')
...
async function createJournalEntry(userId, content, dateKey) {
  const now = new Date()
  const trimmed = sanitizeJournalContent(content)
  if (!trimmed) throw new Error('EMPTY_JOURNAL')
  const stored = encrypt(trimmed)
  if (await hasEntryDateColumn()) {
    const r = await db.query(
      `INSERT INTO diary (user_id, content, entry_date, created_at)
       VALUES ($1, $2, $3::date, $4)
       RETURNING id, content, entry_date, created_at`,
      [userId, stored, dateKey, now],
    )
...
async function updateJournalEntry(id, userId, content, dateKey = null) {
  const trimmed = sanitizeJournalContent(content)
  if (!trimmed) throw new Error('EMPTY_JOURNAL')
  const stored = encrypt(trimmed)
  if (await hasEntryDateColumn()) {
    const r = dateKey
      ? await db.query(
          'UPDATE diary SET content = $1, entry_date = $2::date WHERE id = $3 AND user_id = $4 RETURNING id, content, entry_date, created_at',
          [stored, dateKey, id, userId],
        )
      : await db.query(
          'UPDATE diary SET content = $1 WHERE id = $2 AND user_id = $3 RETURNING id, content, entry_date, created_at',
          [stored, id, userId],
        )
    return serializeJournalRow(r.rows[0])
  }
  const r = await db.query(
    'UPDATE diary SET content = $1 WHERE id = $2 AND user_id = $3 RETURNING id, content, created_at',
    [stored, id, userId],
  )
...
async function deleteJournalEntry(id, userId) {
  await db.query('DELETE FROM diary WHERE id = $1 AND user_id = $2', [id, userId])
}
